...
The plots below are generated directly from the RTL simulation output using the scripts in `python/`.

| Continuous Intensity Curve | RTL Log-Likelihood Scores |
|:---:|:---:|
| ![Hawkes continuous intensity curve](python/hawkes_continuous_curve.png) | ![RTL log-likelihood scores](python/rtl_ll_scores_plot.png) |
| Reconstructed continuous-time Hawkes intensity, generated by `fpga_hawkes_viz.py` from the RTL simulation trace | Log-likelihood scores computed on-chip by the three parallel engines (Quiet, Trending, Crash), decoded from the ILA capture via `decode_ila_capture.py` |
